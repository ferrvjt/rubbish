...
static func cargar_partida() -> EstadoJuego:
	...
	estado_cargado.turno = datos.get("turno", 1)
	estado_cargado.cartas_a_robar = datos.get("cartas_a_robar", 4)
	estado_cargado.limite_mano = datos.get("limite_mano", 10)
	estado_cargado.estado_partida = datos.get("estado_partida", "JUGANDO")
	
	if datos.has("logs"):
		estado_cargado.logs.clear()
		for l in datos["logs"]:
			estado_cargado.logs.append(str(l))
